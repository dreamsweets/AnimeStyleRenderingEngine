...
float3 HitPosition()
{
    return WorldRayOrigin() + WorldRayDirection() * (RayTCurrent() - 0.005);
}
...
[shader("raygeneration")] // Raygen 0
void RayGeneration_01_Default()
{
    uint2 screen_idx = DispatchRaysIndex().xy;

    OuputTexture[screen_idx] = float4(ShootCameraRay().color + 0.1f, 1.0f);
}
