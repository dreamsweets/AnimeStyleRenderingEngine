...
StructuredBuffer<matrix> g_OffsetArray;
StructuredBuffer<FrameTrans> g_FrameTransArray;
StructuredBuffer<FrameTrans> g_ChangeFrameTransArray;

RWStructuredBuffer<matrix> g_BoneMatrixArray;

cbuffer AnimationUpdateCBuffer
{
    int g_AnimBoneCount;
...
void CSMain(int3 ThreadID : SV_DispatchThreadID)
{
    ...
    uint currentFrameIndex = g_AnimCurrentFrame * g_AnimBoneCount + ThreadID.x;
    uint nextFrameIndex = g_AnimNextFrame * g_AnimBoneCount + ThreadID.x;
    
    float4 Scale = lerp(g_FrameTransArray[currentFrameIndex].Scale, g_FrameTransArray[nextFrameIndex].Scale, g_AnimRatio);
    float4 Pos = lerp(g_FrameTransArray[currentFrameIndex].Translate, g_FrameTransArray[nextFrameIndex].Translate, g_AnimRatio);
    float4 Rot = QuternionLerp(g_FrameTransArray[currentFrameIndex].Rot, g_FrameTransArray[nextFrameIndex].Rot, g_AnimRatio);

    if (g_AnimChangeAnimation == true)
    {
        uint ChangeFrameIndex = ThreadID.x * g_AnimFrameCount;
